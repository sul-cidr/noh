...
<!--{% include kotsuzumi-sounds.html %}-->
    ...
    <p>Each of these calls can take individual shape depending on context. They not only help set the proper mood, they are also an important communication tool, helping the musicians coordinate acceleration and slow down. The example below is a shorter version of <em>ashirai</em> played with expression appropriate for the play Aoi no ue. It illustrates how the two percussionists use <em>kakegoe</em> to manage changes of tempo. As seen in the video the ōtsuzumi and kotsuzumi alternate their calls with drumming strokes. Because the ōtsuzumi player initiates patterns he has a leading role in establishing the tempo and mood but both players influence each other in creating a cumulative tempo.</p>
    ...
    1.	The two percussion instruments have a large selection of <em>mitsuji</em> and <em>tsuzuke</em> patterns (well beyond the few provided examples). In our narrative, we have used these terms broadly: <em>mitsuji</em>, for an eight-beat measure (honji) articulated with a limited number of strokes; and <em>tsuzuke</em> for a <em>honji</em> articulated typically with one stroke per beat.<br>
    ...
    <p>When performing dances and some entrance music, the complementarity between the two hand drums is somewhat relaxed as they perform patterns of different length that do not necessarily interlock within an eight-beat unit. This is demonstrated in the following excerpts from the dance <em>jo no mai</em>.
    In this example the kotsuzumi’s <em>nagaji</em> pattern is spread over sixteen-beats against the ōtsuzumi's <em>ji</em> pattern that lasts eight-beat. The repetiions of patterns of the flute and two hand drums become almost hypnotic bringing them to the background and leaving the foreground to the dance.</p>
